...
def main():
    ...
    set_payload("software engineer","pune","1")
    set_last_url(Souper(url, payload).get_last_url())  # setting initial l_url
    ...
    if len(jobCards) == 0:
        ...
        if len(other_job) != 0:
            ...
            set_last_url(Souper(url, payload).get_last_url())
            jobCards = soup.findAll("div", {"class": "jobsearch-SerpJobCard"})
...
# job array; containing Indeed_Job objects
job_arr = []
...
def ret_json_arr():
    return json_arr,indeed_job_count


print("[+]Indeed Jobs Scraped: ",indeed_job_count," [+]")
for i in json_arr:
    a = json.loads(i)
    print("Title: ",a["title"])
    print("Employer: ",a["employer"])
    print("Address: ",a["address"])
    print("date: ",a["dateposted"])
    print("link: ",a["link"])
    print('''
